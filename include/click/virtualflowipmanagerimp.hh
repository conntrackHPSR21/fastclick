 ...
 *	    IMP methods would set to to nolock.
...
const MaintainerArg maintainerArgNone { false, false, false };
const MaintainerArg maintainerArgMP { true, false, false };
const MaintainerArg maintainerArgIMP { true, true, false };
const MaintainerArg maintainerArgIMPNOFID { true, true, true };
...
class VirtualFlowIPManagerIMP : public VirtualFlowManager,
  ...
  protected:
    ...
    virtual int find(IPFlow5ID &f, int core) { return -1; }
    virtual int insert(IPFlow5ID &f, int flowid, int core) { return -1; }
    virtual int find(IPFlow5ID &f, int core, Packet *p) {
        return find(f, core);
    }
    virtual int insert(IPFlow5ID &f, int flowid, int core, Packet *p) {
        return insert(f, flowid, core);
    }
    ...
    virtual inline int get_count(int core);
    ...
    // Deletion methods
    // By default, do nothing on deletion
    virtual inline int delete_flow(FlowControlBlock *fcb, int core = 0) {
        return 0;
    };
...
inline void VFIM_CLASS::process(Packet *p, BatchBuilder &b, Timestamp &recent,
    ...
    if (ret == 0) {
        ...
        *(get_flowid(fcb)) = flowid;
        ...
        if (_timeout) memcpy(get_fid(fcb), &fid, sizeof(IPFlow5ID));
